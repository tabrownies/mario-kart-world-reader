...
st.write("Welcome to the camera picker! Use the drop-down to select a video capture device.")
...
if camera_idx_in_vid_idxs:
    ...
    st.session_state['camera_idx'] = st.selectbox("Select camera index", vid_indices, index = vid_idx)
    try:
        camera = OBSVirtualCameraStream(camera_index=st.session_state["camera_idx"])
        frame = camera.get_frame()
        st.image(frame)
    except Exception as e:
        st.write(f"Unable to capture camera frame with camera frame {st.session_state['camera_idx']} {str(e)}")
else:
