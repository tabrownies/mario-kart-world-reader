...
csv_files = [
    path
    for path in os.listdir(TRAINING_CSV_DIR)
    if (path.endswith(".csv") and (not path.startswith("UNFINISHED")))
]
...
chosen_video = st.selectbox(
    label="Select Video to Label:", options=unfinished_video_files, index=None
)
...
if "loaded_video" not in st.session_state:
    with st.spinner():
        st.session_state["loaded_video"] = cv2.VideoCapture(
            os.path.join(VIDEO_DIR, chosen_video)
        )
...
def left_pressed():
    st.session_state["frameidx"] = max(
        0, st.session_state["frameidx"] - st.session_state["frameskip"]
    )


def right_pressed():
    st.session_state["frameidx"] = min(
        video_frame_count, st.session_state["frameidx"] + st.session_state["frameskip"]
    )
...
with left:
    st.session_state["frameidx"] = st.number_input(
        label="Frame Index",
        min_value=0,
        max_value=video_frame_count,
        value=st.session_state["frameidx"],
    )

with right:
    st.session_state["frameskip"] = st.number_input(
        label="Frame skip", min_value=1, value=st.session_state["frameskip"]
    )
...
loaded_video.set(
    ...
    max(0, min(st.session_state["frameidx"], video_frame_count)),
)
...
key_frame_type = st.selectbox(
    label="Select Key Frame Type", options=["place", "items", "coins"]
)
...
if key_frame_type == "place":
    ...
    to_place = st.selectbox(
        label="Place Value Moving TO (0 is unknown)",
    ...
    def add_place_frame():
        ...
        st.toast(
            f"added key frame {st.session_state['frameidx']} with value {st.session_state['placekeyframes'][st.session_state['frameidx']]}"
        )
    ...
    remove_frame_idx = st.selectbox(
        label="Select Key Frame to Remove",
        options=st.session_state["placekeyframes"],
        index=None,
    )
